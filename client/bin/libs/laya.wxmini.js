(function (window, document, Laya) {
  ...
  var MiniAdpter = (function () {
    ...
    MiniAdpter.init = function (isPosMsg, isSon) {
      ...
      if (!MiniAdpter.isZiYu) {
        ...
        MiniFileMgr.existDir(
          MiniFileMgr.fileNativeDir,
          Handler.create(MiniAdpter, MiniAdpter.onMkdirCallBack)
        );
      }
      ...
      MiniAdpter.window.CanvasRenderingContext2D.prototype = MiniAdpter.window.wx
        .createCanvas()
        ...
        MiniAdpter.window.wx.onMessage(function (message) {
          if (message['isLoad'] == 'opendatacontext') {
            if (message.url) {
              ...
              MiniFileMgr.ziyuFileTextureData[message.imgReadyUrl] =
                message.imgNativeUrl;
            }
    ...
    MiniAdpter.clearFileList = function () {
    ...
    MiniAdpter.parseXMLFromString = function (value) {
      ...
      try {
        /*__JS__ */ rst = new window.Parser.DOMParser().parseFromString(
          value,
          'text/xml'
        );
      } catch (error) {
  ...
  var MiniFileMgr = (function () {
    ...
    MiniFileMgr.read = function (
      ...
      if (
        readyUrl != '' &&
        (readyUrl.indexOf('http://') != -1 ||
          readyUrl.indexOf('https://') != -1)
      ) {
      ...
      MiniFileMgr.fs.readFile({
        ...
        fail: function (data) {
          ...
          else callBack != null && callBack.runWith([1]);
        },
      });
    };

    MiniFileMgr.downFiles = function (
      fileUrl,
      encoding,
      callBack,
      readyUrl,
      ...
      if (
        fileUrl.startsWith(MiniAdpter.window.wx.env.USER_DATA_PATH + '/cache/')
      ) {
        MiniFileMgr.readFileOnly(fileUrl, encoding, callBack, fileType);
        return;
      }
    ...
    MiniFileMgr.readFileOnly = function (
      filePath,
      encoding,
      callBack,
      fileType
    ...
    MiniFileMgr.downOtherFiles = function (
      fileUrl,
      callBack,
      readyUrl,
      isSaveFile,
      ...
      MiniFileMgr.wxdown({
        ...
        success: function (data) {
          if (data.statusCode === 200) {
            if (
              (MiniAdpter.autoCacheFile || isSaveFile) &&
              readyUrl.indexOf('wx.qlogo.cn') == -1 &&
              readyUrl.indexOf('thirdqq.qlogo.cn') == -1 &&
              readyUrl.indexOf('thirdwx.qlogo.cn') == -1 &&
              readyUrl.indexOf('.php') == -1
            ) {
    ...
    MiniFileMgr.downLoadFile = function (
      fileUrl,
      fileType,
      callBack,
      encoding
      ...
      if (MiniAdpter.window.navigator.userAgent.indexOf('MiniGame') < 0) {
        ...
        if (
          fileType == /*laya.net.Loader.IMAGE*/ 'image' ||
          fileType == /*laya.net.Loader.SOUND*/ 'sound'
        )
    ...
    MiniFileMgr.deleteFile = function (
      tempFileName,
      readyUrl,
      callBack,
      encoding,
    ...
    MiniFileMgr.deleteAll = function () {
      ...
      if (
        laya.wx.mini.MiniFileMgr.filesListObj &&
        laya.wx.mini.MiniFileMgr.filesListObj.fileUsedSize
      ) {
    ...
    MiniFileMgr.onSaveFile = function (
      readyUrl,
      md5Name,
      isAdd,
      encoding,
      ...
      if (isAdd) {
        ...
        MiniFileMgr.filesListObj['fileUsedSize'] =
          parseInt(MiniFileMgr.filesListObj['fileUsedSize']) + fileSize;
        MiniFileMgr.writeFilesList(
          fileurlkey,
          JSON.stringify(MiniFileMgr.filesListObj),
          true
        );
        callBack != null && callBack.runWith([0]);
      } else {
        if (MiniFileMgr.filesListObj[fileurlkey]) {
          var deletefileSize = parseInt(
            MiniFileMgr.filesListObj[fileurlkey].size
          );
          MiniFileMgr.filesListObj['fileUsedSize'] =
            parseInt(MiniFileMgr.filesListObj['fileUsedSize']) - deletefileSize;
    ...
    MiniFileMgr.writeFilesList = function (fileurlkey, filesListStr, isAdd) {
      var listFilesPath =
        MiniFileMgr.fileNativeDir + '/' + MiniFileMgr.fileListName;
      ...
      MiniFileMgr.fs.access({
        ...
        fail: function () {
          MiniFileMgr.fs.mkdir({
            ...
            success: function () {
              callBack != null &&
                callBack.runWith([0, { data: JSON.stringify({}) }]);
            },
            fail: function (data) {
              callBack != null && callBack.runWith([1, data]);
            },
    ...
    MiniFileMgr.setNativeFileDir = function (value) {
      MiniFileMgr.fileNativeDir =
        MiniAdpter.window.wx.env.USER_DATA_PATH + value;
    };
  ...
  var MiniImage = (function () {
    ...
    __proto._loadImage = function (url) {
      ...
      if (!MiniFileMgr.isLocalNativeFile(url)) {
        ...
        if (
          ...
          MiniAdpter.subNativeheads.length == 0
        ) {
          for (var key in MiniAdpter.subNativeFiles) {
            var tempArr = MiniAdpter.subNativeFiles[key];
            MiniAdpter.subNativeheads = MiniAdpter.subNativeheads.concat(
              tempArr
            );
        ...
        if (MiniAdpter.subNativeFiles && url.indexOf('/') != -1) {
          ...
          if (
            curfileHead &&
            MiniAdpter.subNativeheads.indexOf(curfileHead) != -1
          ) {
      ...
      if (!MiniFileMgr.getFileInfo(url)) {
        if (url.indexOf('http://') != -1 || url.indexOf('https://') != -1) {
          if (
            MiniAdpter.isZiYu ||
            url.indexOf('pstatp.com') != -1 ||
            url.indexOf('bytecdn.cn') != -1 ||
            url.indexOf('byteimg.com') != -1 ||
            url.indexOf('hdslb.com') != -1
          ) {
            MiniImage.onCreateImage(url, thisLoader, true);
          } else {
            MiniFileMgr.downOtherFiles(
              url,
              new Handler(MiniImage, MiniImage.onDownImgCallBack, [
                url,
                thisLoader,
              ]),
              url
    ...
    MiniImage.onDownImgCallBack = function (
      sourceUrl,
      thisLoader,
      errorCode,
      tempFilePath
    ...
    MiniImage.onCreateImage = function (
      sourceUrl,
      thisLoader,
      isLocal,
      tempFilePath
      ...
      if (MiniAdpter.autoCacheFile) {
        ...
        if (!isLocal) fileNativeUrl = tempFilePath;
        else fileNativeUrl = sourceUrl;
      }
  ...
  var MiniInput = (function () {
    ...
    MiniInput._createInputElement = function () {
      ...
      /*__JS__ */ wx.onWindowResize &&
        /*__JS__ */ wx.onWindowResize(function (res) {
          /*__JS__ */ window.dispatchEvent &&
            /*__JS__ */ window.dispatchEvent('resize');
        });
    ...
    MiniInput._onStageResize = function () {
      ...
      ts.scale(
        Browser.width / Render.canvas.width / RunDriver.getPixelRatio(),
        Browser.height / Render.canvas.height / RunDriver.getPixelRatio()
      );
    };
  ...
  var MiniLocation = (function () {
    ...
    MiniLocation.__init__ = function () {
      MiniAdpter.window.navigator.geolocation.getCurrentPosition =
        MiniLocation.getCurrentPosition;
      MiniAdpter.window.navigator.geolocation.watchPosition =
        MiniLocation.watchPosition;
      MiniAdpter.window.navigator.geolocation.clearWatch =
    ...
    MiniLocation._myLoop = function () {
      MiniLocation.getCurrentPosition(
        MiniLocation._mySuccess,
        MiniLocation._myError
      );
    };
  ...
  var MiniAccelerator = (function (_super) {
    ...
    MiniAccelerator.startListen = function (callBack) {
      ...
      try {
        MiniAdpter.window.wx.onAccelerometerChange(
          laya.wx.mini.MiniAccelerator.onAccelerometerChange
        );
      } catch (e) {}
    };
  ...
  var MiniLoader = (function (_super) {
    ...
    __proto.load = function (url, type, cache, group, ignoreCache) {
      ...
      if (url.indexOf('data:image') === 0)
        thisLoader._type = type = /*laya.net.Loader.IMAGE*/ 'image';
      else {
      ...
      if (
        MiniLoader._fileTypeArr.indexOf(urlType) != -1 ||
        type == /*laya.net.Loader.IMAGE*/ 'image'
      ) {
        MiniAdpter.EnvConfig.load.call(
          this,
          url,
          type,
          cache,
        ...
        if (!MiniFileMgr.getFileInfo(url)) {
          if (MiniFileMgr.isLocalNativeFile(url)) {
            ...
            if (type == /*laya.net.Loader.SOUND*/ 'sound') {
              ...
              MiniFileMgr.read(
                url,
                encoding,
                new Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [
                  encoding,
                  url,
                  type,
                  cache,
          ...
          if (
            url.indexOf('http://') != -1 ||
            (url.indexOf('https://') != -1 && !MiniAdpter.AutoCacheDownFile)
          ) {
            if (type == /*laya.net.Loader.SOUND*/ 'sound') {
              thisLoader._loadSound(url);
            } else {
            ...
            if (fileObj) {
              fileObj.encoding =
                fileObj.encoding == null ? 'utf8' : fileObj.encoding;
              MiniFileMgr.readFile(
                fileObj.url,
                encoding,
                new Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [
              ...
              thisLoader.type == 'image' ||
              thisLoader.type == 'htmlimage'
            ) {
              MiniAdpter.EnvConfig.load.call(
                thisLoader,
                url,
              ...
              if (
                ...
                ((url.indexOf('http://') == -1 &&
                  url.indexOf('https://') == -1) ||
                  MiniFileMgr.isLocalNativeFile(url))
              ) {
                MiniFileMgr.readFile(
                  url,
                  encoding,
    ...
    __proto._loadSound = function (url) {
      ...
      if (MiniFileMgr.isLocalNativeFile(url)) {
        ...
        if (
          !MiniFileMgr.isLocalNativeFile(url) &&
          tempurl.indexOf('http://') == -1 &&
          tempurl.indexOf('https://') == -1
        ) {
          laya.wx.mini.MiniLoader.onDownLoadCallBack(url, thisLoader, 0);
        } else {
          MiniFileMgr.downOtherFiles(
            ...
            Handler.create(
              MiniLoader,
              laya.wx.mini.MiniLoader.onDownLoadCallBack,
              [tempurl, thisLoader]
            ),
            tempurl
          );
    ...
    MiniLoader.onDownLoadCallBack = function (
      sourceUrl,
      thisLoader,
      errorCode,
      tempFilePath
    ...
    MiniLoader.onReadNativeCallBack = function (
      encoding,
      url,
      type,
      cache,
      ...
      if (!errorCode) {
        var tempData;
        if (
          type == /*laya.net.Loader.JSON*/ 'json' ||
  ...
  var MiniSound = (function (_super) {
      ...
      if (MiniAdpter.autoCacheFile && MiniFileMgr.getFileInfo(url)) {
        ...
        if (!MiniAdpter.autoCacheFile) {
          ...
          if (MiniFileMgr.isLocalNativeFile(url)) {
            ...
            MiniFileMgr.downOtherFiles(
              encodeURI(url),
              Handler.create(this, this.onDownLoadCallBack, [url]),
              url
            );
          }
        }
    ...
    __proto.play = function (startTime, loops) {
      ...
      if (
        ...
        !SoundManager._tMusic.startsWith(
          MiniAdpter.window.wx.env.USER_DATA_PATH + '/cache/'
        )
      ) {
