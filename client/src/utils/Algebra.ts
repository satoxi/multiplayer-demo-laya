namespace Muse {
  ...
  export class Vector implements IVector {
    ...
    public static sqrDistance(vec1: Vector, vec2: Vector) {
      return Math.pow(vec1.x - vec2.x, 2) + Math.pow(vec1.y - vec2.y, 2);
    }

    public static perpendicular(first: Vector, second: Vector): Vector {
      return new Vector(-1 * (second.y - first.y), second.x - first.x);
    }

    public static unsignedAngle(
      from: Vector,
      to: Vector,
      round: boolean = true
    ) {
      ...
      to = to.normalize();
      const angle =
        Math.acos(MathUtils.clamp(from.dot(to), -1, 1)) * MathUtils.rad2Deg;
      return round ? Math.round(angle) : angle;
    }

    public static signedAngle(from: Vector, to: Vector) {
      from = from.normalize();
      to = to.normalize();
      return (
    ...
    public static lerp(from: Vector, to: Vector, t: number) {
      return new Vector(
     ...
     * If no other Vector is specified, this will return the [[magnitude]].
    ...
    public sqrDistance(v?: Vector): number {
      if (!v) {
        v = Vector.zero;
      }
    ...
    /**
     * Subtracts a vector from another, if you subract vector `B.sub(A)`
     * the resulting vector points from A -> B
    ...
    /**
     * Performs a 2D cross product with scalar.
     * 2D cross products with a scalar return a vector.
     * @param v  The scalar to cross
     */
    public cross(v: number): Vector;
    /**
     * Performs a 2D cross product with another vector.
     * 2D cross products return a scalar value not a vector.
    ...
    public transform(matrix: Matrix2D): Vector {
      return new Vector(
  ...
  export class Ray2D {
    ...
    public constructor(pos: Vector, end: Vector) {
  ...
  export class Line {
    ...
    /**
     * Finds a point on the line given only an X or a Y value.
     * Given an X value, the function returns
    ...
    /**
     * Whether or not the given point lies on this line.
     * This method is precise by default
    ...
    public hasPoint(): boolean {
      ...
      if (
        ...
        throw new Error(
          'Could not determine the arguments for Vector.hasPoint'
        );
      }
