namespace Muse {
  ...
  export class Color {
    ...
    public static hexColorToFilter(hexColor: string): Laya.ColorFilter {
      const matrix = [];
      for (let i = 0; i < 20; i++) {
        matrix.push(i === 18 ? 1 : 0);
      }

      const color = Muse.Color.createFromHex(hexColor);

      matrix[0] = color.r / 256;
      matrix[5] = color.g / 256;
      matrix[10] = color.b / 256;

      return new Laya.ColorFilter(matrix);
    }
